...
long long candidate_sub_array = 0;
long long usage_sub_array = 0;
long long identification_sub_array = 0;
long long memory_sub_array = 0;
...
public:
    ...
    sub_array(){
        ++identification_sub_array;
        ++memory_sub_array;
        maxx_cnt_sub_array = max(maxx_cnt_sub_array,memory_sub_array);
        pre = nullptr;
    }
...
void Modify(int r){
    ...
    for(int i=0;i<=siz;++i){
        ...
        if(now->end <= r){
            ...
            memory_sub_array -= now->subs.size();
...
int main(int argc, char* argv[])
{
    ...
    freopen("Memory_Candidate_Streaming_RT.txt","a",stdout);
    ...
    printf("average_candidate=%.4lf\n",(1.0*usage_sub_array)/(1.0*cnt_window));
    printf("average_identification_candidate_per_window=%.4lf\n",(1.0*identification_sub_array)/(1.0*cnt_window));
    printf("average_comparision_candidate_per_window=%.4lf\n",(1.0*usage_sub_array)/(1.0*cnt_window));
    printf("average_memory_usage_per_window=%.4lf",(1.0*tot_subarray_per_window*sizeof(sub_array))/(1.0*cnt_window));
    return 0;
}