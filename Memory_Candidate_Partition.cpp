...
#define ll long long
long long candidate_partition = 0;
long long candidate_crest = 0;
ll n,w,s;
int k=0;
...
void Get_TopK_crests_new(Crest* crest,int mod,Partition* partition){
    if(mod==1) candidate_crest++;
    ...
    for(int i=crest->merge_crests.size()-1;i>=0;--i){
        nowCrest = crest->merge_crests.at(i);
        if(nowCrest->leftBound < crest->domainCrest){
            Get_TopK_crests_new(nowCrest,1,partition);
        }
    }
}
...
long long memory_crest = 0;
void Get_TopK_crests_new1(Crest* crest,int mod,Partition* partition){
    ++memory_crest;
    Crest* nowCrest;

    for(int i=crest->merge_crests.size()-1;i>=0;--i){
        nowCrest = crest->merge_crests.at(i);
        Get_TopK_crests_new1(nowCrest,1,partition);
    }
}
...
void get_tot(){
    candidate_partition += S_MaxPartition->size();
    memory_partition += S_MaxPartition->size();
    for(auto partition : *(S_MaxPartition)){
        for(auto crest : *(partition->merge_crests)){
            Get_TopK_crests_new1(crest,1,partition);
            if(!partition->deleted) Get_TopK_crests_new(crest,1,partition);
        }
    }
}
...
//    freopen("Memory_Candidate_Partition.txt","a",stdout);
...
//    printf("cnt_partition=%lld\n",cnt_partition);
//    printf("cnt_crest=%lld\n",cnt_create_crest);
    double x3 = (1.0*candidate_crest+1.0*candidate_partition);
    printf("average_candidate=%.4lf\n",x3/cnt_window);
    double x4=1.0*cnt_partition+1.0*cnt_create_crest;
    printf("average_identification_candidate_per_window=%.4lf\n",x4/cnt_window);
    double x2=(1.0*cnt_use)/(1.0*cnt_window);
    printf("long long=%lld\n",cnt_use);
    printf("average_comparision_candidate_per_window=%.4lf\n",x2);
    double x1=memory_partition*1.0*sizeof(Partition) + memory_crest*1.0*sizeof(Crest);
    printf("average_memory_usage_per_window=%.4lf",x1 / cnt_window / 1024.0 / 1024.0);
    return 0;
}  